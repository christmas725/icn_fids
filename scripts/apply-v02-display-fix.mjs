...
function patchBoard() {
  ...
  text = replaceOnce(
    ...
    `const DEFAULT_PAGE_SIZE = 14;\nconst LARGE_TABLET_LANDSCAPE_PAGE_SIZE = 16;\nconst LARGE_TABLET_PORTRAIT_PAGE_SIZE = 20;\nconst MAX_PAGES = 4;`,
  ...
  const clockAndTabletEffects = `${clockEffect}\n\n  useEffect(() => {\n    const landscapeTablet = window.matchMedia(\n      "(min-width: 1101px) and (max-width: 1700px) and (hover: none), (min-width: 1101px) and (max-width: 1700px) and (pointer: coarse)"\n    );\n    const portraitTablet = window.matchMedia(\n      "(min-width: 800px) and (max-width: 1100px) and (orientation: portrait) and (max-aspect-ratio: 3/4) and (hover: none), (min-width: 800px) and (max-width: 1100px) and (orientation: portrait) and (max-aspect-ratio: 3/4) and (pointer: coarse)"\n    );\n\n    const updatePageSize = () => {\n      if (portraitTablet.matches) {\n        setPageSize(LARGE_TABLET_PORTRAIT_PAGE_SIZE);\n      } else if (landscapeTablet.matches) {\n        setPageSize(LARGE_TABLET_LANDSCAPE_PAGE_SIZE);\n      } else {\n        setPageSize(DEFAULT_PAGE_SIZE);\n      }\n    };\n\n    updatePageSize();\n    landscapeTablet.addEventListener("change", updatePageSize);\n    portraitTablet.addEventListener("change", updatePageSize);\n    return () => {\n      landscapeTablet.removeEventListener("change", updatePageSize);\n      portraitTablet.removeEventListener("change", updatePageSize);\n    };\n  }, []);`;
  text = replaceOnce(text, clockEffect, clockAndTabletEffects, "adaptive tablet media detection");
...
function patchRoute() {
  ...
  const newConstants = `// 기본 화면은 14행, 대형 태블릿 가로는 16행, 세로는 20행으로 최대 4페이지 운용한다.\n// 서버는 가장 큰 세로 태블릿 기준으로 T1/T2 각각 최대 80개 실제 운항을 확보한다.\n// 다른 기기는 클라이언트에서 자신의 페이지 크기까지만 사용한다.\nconst DISPLAY_HORIZON_MINUTES = 24 * 60;\nconst TARGET_OPERATIONS_PER_TERMINAL = 80;\nconst DEPARTED_GRACE_MS = 5 * 60 * 1000;`;
  ...
  text = text
    ...
    .replace("각 터미널별 첫 30개 실제 운항 묶음", "각 터미널별 첫 80개 실제 운항 묶음")
    .replace("각 터미널별 첫 60개 실제 운항 묶음", "각 터미널별 첫 80개 실제 운항 묶음")
    .replace("각 터미널별 첫 56개 실제 운항 묶음", "각 터미널별 첫 80개 실제 운항 묶음")
    .replace("각 터미널별 첫 64개 실제 운항 묶음", "각 터미널별 첫 80개 실제 운항 묶음")
    ...
    .replace("T1/T2 각각 최대 30운항(15편 × 2페이지)", "T1/T2 각각 최대 80운항(20편 × 4페이지)")
    .replace("T1/T2 각각 최대 60운항(15편 × 4페이지)", "T1/T2 각각 최대 80운항(20편 × 4페이지)")
    .replace("T1/T2 각각 최대 56운항(14편 × 4페이지)", "T1/T2 각각 최대 80운항(20편 × 4페이지)")
    .replace("T1/T2 각각 최대 64운항(16편 × 4페이지)", "T1/T2 각각 최대 80운항(20편 × 4페이지)");
...
patchBoard();
patchRoute();
console.log("ICN FIDS v0.2 adaptive 14/16/20-row display fix applied");
